  ...
  useEffect(() => {
    const fetchPreview = async (auditId: string) => {
      try {
        ...
        dispatch({
          type: PreviewActionType.ADD_PREVIEW,
          payload: currentPreview,
        });
      } catch (error) {
        toast.error(`Failed to fetch preview: ${error}.`);
      } finally {
        setIsLoading(false);
      }
    };
        ...
        <div className="flex min-h-screen flex-col">
          <header className="sticky top-0 z-40 w-full border-b bg-background">
            <div className="container flex h-16 items-center space-x-4 sm:justify-between sm:space-x-0">
              <MainNav items={dashboardConfig.mainNav}></MainNav>
          ...
          <div className="container flex-1">
            <div className="flex-1 md:grid md:grid-cols-[220px_1fr] md:gap-6 lg:grid-cols-[240px_1fr] lg:gap-10"></div>
          </div>
