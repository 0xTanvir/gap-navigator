  ...
  return (
      <>
        <div className="flex-1 space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            <DashboardCard
                title={`Total ${dashboardData.consultantCounts > 1 ? 'Consultants' : 'Consultant'}`}
                totalNumber={dashboardData.consultantCounts} iconName="users"
            />
            <DashboardCard
                title={`Total ${dashboardData.clientCounts > 1 ? 'Clients' : 'client'}`}
                totalNumber={dashboardData.clientCounts} iconName="users"
            />
            <DashboardCard
                title={`Total ${dashboardData.auditsCounts > 1 ? 'Audits' : 'Audit'}`}
                totalNumber={dashboardData.auditsCounts} iconName="audit"
            />
          </div>
        </div>
